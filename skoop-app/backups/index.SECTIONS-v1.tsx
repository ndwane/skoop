...
const CATEGORIES = [
  { id: 'cars', label: 'قطع سيارات', icon: 'car' },
  { id: 'motorcycles', label: 'قطع دراجات نارية', icon: 'bicycle' },
  { id: 'bicycles', label: 'قطع سياكل هوائية', icon: 'bicycle-outline' },
  { id: 'rc_planes', label: 'طائرات تحكم RC', icon: 'airplane', isNew: true },
  { id: 'rc_cars', label: 'سيارات تحكم RC', icon: 'car-sport', isNew: true },
];
...
export default function Index() {
  ...
  const S = StyleSheet.create({
    ...
    catCard: { width: '47.5%', backgroundColor: CT.card, borderRadius: 16, borderWidth: 0.5, borderColor: CT.cardBorder, paddingVertical: 26, paddingHorizontal: 10, alignItems: 'center', position: 'relative' },
        ...
        {CATEGORIES.map((cat, idx) => {
          ...
          return (
            <TouchableOpacity key={cat.id} style={[S.catCard, fullWidth && { width: '100%' }]} activeOpacity={0.7} onPress={() => { setSelectedCategory(cat.id); setFilterCity(''); setSearch(''); }}>
              ...
              <Ionicons name={cat.icon} size={40} color={CT.navy} />
